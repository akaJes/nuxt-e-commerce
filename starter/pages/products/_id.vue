...
<div v-if="product != null">
  <div class="m-6 mt-56 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-4 mt-8">
    ...
    <div class="w-full p-5 flex flex-col justify-between">
      ...
      <button v-if="product.attributes.status === 'published'" class="snipcart-add-item mt-4 bg-white border border-gray-200 d hover:shadow-lg text-gray-700 font-semibold py-2 px-4 rounded shadow" :data-item-id="product.id" :data-item-price="product.attributes.price"
...
export default {
  ...
  async mounted() {
    try {
      this.product = (await this.$strapi.findOne('products', this.$route.params.id, { populate: '*' })).data
    } catch (error) {
      this.error = error
    }
  },
  computed: {
    customFields() {
      return this.product.attributes["Custom_field"]
        .map(({
          title,
          required,
          options
